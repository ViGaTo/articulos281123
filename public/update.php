...
if(!Articulos::recogerArticuloId($_GET['id'])){
    header("Location:index.php");
    die();
}
...
if(isset($_POST['btn'])){
    ...
    if(Tools::existeNombre($nombre, $id)){
        $errores = true;
    }
    ...
    $_SESSION['mensaje'] = "Exito: El articulo se ha modificado";
    header("Location:index.php");
}
?>
<!DOCTYPE html>
...
<body>
    <h3 class="my-2 text-xl text-center">ACTUALIZAR ARTICULO</h3>
    <div class="w-1/2 mx-auto p-4 rounded-xl shadow-xl bg-white border border-gray-200">
        <form action="update.php?id=<?php echo $id?>" method="POST" enctype="multipart/form-data">
            <div class="mb-4">
                ...
                <input value="<?php echo $articulo->nombre?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nombre" type="text" placeholder="Nombre del articulo..." name="nombre">
            <?php
            ...
            <div class="mb-4">
                ...
                <input value="<?php echo $articulo->precio?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="precio" type="text" placeholder="Precio..." name="precio">
                <?php
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="category_id" name="category_id">
                    ...
                    foreach ($categorias as $item) {
                       $seleccion = ($item->id == $articulo->category_id) ? "selected" : "";
                        echo <<<TXT
                        <option value='{$item->id}' $seleccion>{$item->nombre}</option>
                        </div>
                        TXT;
                    }
                    ?>
                ...
                <div class="mb-4">
                        <label class="relative inline-flex items-center mb-4 cursor-pointer">
                            <?php $disponible = ($articulo->disponible=="SI") ? "checked": "";
                            echo <<<TXT
                            <input type="checkbox" $disponible value="Disponible" class="sr-only peer" name="disponible">
